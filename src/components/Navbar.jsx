          ...
          <nav>
            ...
            <div className='hidden sm:block'>
              ...
              <ul className='flex items-center gap-3 sm:gap-1 lg:gap-3 justify-around pr-7 lg:pl-16 lg:pr-16'>
                {navLinks.map((link) => {
                  return (<li key={link} className='cursor-pointer'>{link}</li>)
                })}
              </ul>
            </div>
          </nav>
          ...
          <nav>
            ...
            <div className='z-20 fixed inset-0 bg-black bg-opacity-25'>
              ...
              <div className='flex justify-center pt-12'>
                <ul className='flex flex-col font-semibold items-center justify-center gap-8 bg-white w-80 h-80 rounded-sm'>
                  {navLinks.map((link) => {
                    return (<li key={link} className='cursor-pointer'>{link}</li>)
                  })}
                  <li className='hidden md:block'><ButtonGetStarted color={'bg-orange-600'}/></li>
                </ul>
              </div>
            </div>
          </nav>
